  ...
  _performSmallStep : function(currentEvent, selectedTransitions) {
    ...
    if (!selectedTransitions.isEmpty()) {
      ...
      for (var j = 0, len = statesExited.length; j < len; j++) {
        ...
        //invoke listeners
        // Csaba Bársony
        var additionalParameterData = currentEvent ? currentEvent.data : undefined
        this.emit('onExit',stateExited.id, additionalParameterData)
      ...
      for (var stxIdx = 0, len = sortedTransitions.length; stxIdx < len; stxIdx++) {
        ...
        // Csaba Bársony
        var eventData = currentEvent ? currentEvent.data : undefined
        this.emit('onTransition',transition.source.id,targetIds, stxIdx, eventData);
      ...
      for (var enterIdx = 0, enterLen = statesEntered.length; enterIdx < enterLen; enterIdx++) {
        ...
        // Csaba Bársony
        var additionalParameterData = currentEvent ? currentEvent.data : undefined
        this.emit('onEntry',stateEntered.id, additionalParameterData);
 ...
 */
// export added by Csaba Bársony
export function Statechart(model, opts) {
  opts = opts || {};
